...
#if defined(__MK64FX512__) || defined(__MK66FX1M0__)
		if (chipSelectPin_ == BUILTIN_SDCARD) {
			return (KinetisSDHC_WriteBlock(src, blockNumber) == 0) ? true : false;
		}
#endif
		return SD_writeBlock(blockNumber, src);
	}
...
private:
	...
	// private functions
	// private functions
	bool SD_init(uint8_t sckRateID, pin_t chipSelectPin);
	bool SD_readBlock(uint32_t block, uint8_t* dst);
	bool SD_writeBlock(uint32_t blockNumber, const uint8_t* src);
